...
brms_reg_weibull = function(df_sim, p, g, seed = 123, prior) {
  ...
  priors = c(
    ...
    prior(gamma(1,1), class = shape) # not log scale!! (mean of gamma is a/b)
  )
  
  stanvars = stanvar(c_scale, name = "c_scale")
  vp = switch(prior,
               "horseshoe" = prior(horseshoe(df = 4, par_ratio = 0.2), class = "b"),
  ...
  fit_bayes_aft = brm(
    ...
    #control = list(adapt_delta = 0.97, max_treedepth = 13, stepsize = 0.1),
    seed = seed,
...
run_grid_experiments_mc = function(
  ...
  # Filename for experiment running
  ...
  for (i in 1:nrow(param_grid)) {
    ...
    write_rds(all_results, file = glue("Experiment:_{datetime_start}.rds"))
  }
...
debug_methods(n = 200, p = 200, g = 4, method = "weibull", seed = 42, t_SNR = 10, perf_A = T, nzg = 1, prior = "cauchy") %>%
  system.time()
```
