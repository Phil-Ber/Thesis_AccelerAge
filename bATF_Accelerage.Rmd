...
brms_reg_weibull = function(df_sim, p, g) {
  ...
  priors = c(
    ...
    prior(gamma(log(80), 0.2), class = Intercept), # about human life expectancy
    # prior(normal(log(5), 0.4), class = shape) # Hazard; bad, gamma better?
    prior(gamma(log(10),0.2), class = shape)
  )
  ...
  fit_bayes_aft = brm(
    ...
    control = list(adapt_delta = 0.96, max_treedepth = 15, stepsize = 0.01,
                   metric = "dense_e"),
    threads = threading(4),
    refresh = 100
  )
...
```{r}
debug_methods(n = 250, p = 200, g = 5, method = "weibull", seed = 40, t_SNR = 4, perf_A = T, nzg = 1) %>%
  system.time()
...
```{r}
debug_methods(n = 500, p = 200, g = 5, method = "weibull", seed = 40, t_SNR = 4, perf_A = T, nzg = 1) %>%
  system.time()
