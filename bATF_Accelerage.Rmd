...
brms_reg_weibull = function(df_sim, p, g, seed = 123) {
  ...
  priors = c(
    # prior(normal(0,2), class = "b"), # I took a wide sd due to convergence problems
    prior(horseshoe(df = 3, par_ratio = 0.1, df_slab = 6), class = "b"), # WOW!!!
    prior(normal(log(80), 0.2), class = Intercept), # about human life expectancy
    #prior(normal(50, 50), class = Intercept), #, # uninformative
    # prior(normal(10, 4), class = shape) # Hazard: bad, gamma better?
    prior(gamma(50, 5), class = shape) # not log scale!! (mean of gamma is a/b)
    # prior(normal(50, 50), class = shape) # uninformative
  )
  ...
  fit_bayes_aft = brm(
    ...
    control = list(adapt_delta = 0.95, max_treedepth = 12, stepsize = 0.1),
    seed = seed,
...
debug_methods(n = 400, p = 2000, g = 20, method = "weibull", seed = 42, t_SNR = 2, perf_A = T, nzg = 0.2) %>%
  system.time()
```
...
### n = 250
```{r}
debug_methods(n = 250, p = 4, g = 4, method = "weibull", seed = 40, t_SNR = 0, perf_A = T, nzg = 0.1) %>%
  system.time()
```
...
```{r}
debug_methods(n = 500, p = 20, g = 10, method = "weibull", seed = 40, t_SNR = 0.5, perf_A = T, nzg = 0.5) %>%
  system.time()
```
...
```{r}
debug_methods(n = 1000, p = 20, g = 10, method = "weibull", seed = 40, t_SNR = 8, perf_A = T, nzg = 0.5) %>%
  system.time()
```


## Method: Weibull; p = 100; $n \in \{250, 500\}$; $r_{nz} \in \{1\}$ 

### n = 250

```{r}
debug_methods(n = 250, p = 100, g = 5, method = "weibull", seed = 40, t_SNR = 8, perf_A = T, nzg = 1) %>%
  system.time()
```
...
```{r}
debug_methods(n = 1000, p = 60, g = 5, method = "weibull", seed = 40, t_SNR = 8, perf_A = T, nzg = 0.4) %>%
  system.time()
```
...
```{r}
debug_methods(n = 500, p = 1000, g = 100, method = "weibull", seed = 40, t_SNR = 8, perf_A = T, nzg = 0.1) %>%
  system.time()
```
