...
aft_reg_weibull = function(df_sim, p) {
  ...
  mrl_linp_test = function(tpoint, shape, scale, linpred) {
    ...
    return((mrl_uncon/s_cond) * exp(-linpred))
  }
  ...
  for (i in 1:nrow(df_sim_test)) {
    ...
    # cat("exp(-linpred):", exp(-linpred_aft[i]), "\n")
...
brms_reg_weibull = function(df_sim, p, g, seed = 123, prior) {
  ...
  priors = c(
    prior(normal(log(80), 1), class = Intercept), # about human life expectancy
    prior(gamma(15,1), class = shape) # not log scale!! (mean of gamma is a/b)
  )
  ...
  # Send to mar
  # We considered three data-generating mechanisms: one in which the baseline hazard ℎ0⁡(𝑡) follows a Gompertz distribution and the PH assumption holds (referred to as Gompertz-PH), one in which ℎ0⁡(𝑡) follows a Gompertz distribution and the AFT assumption holds (referred to as Gompertz-AFT) and one in which ℎ0⁡(𝑡) follows a Weibull distribution (referred to as Weibull), which per definition can be parameterized as both an AFT and a PH model at the same time. We included the Gompertz scenarios because the Gompertz distribution is known to accurately describe (human) lifespan. We included the Weibull scenario to illustrate the effect of a misspecified baseline when fitting a parametric AFT model.
  ...
  fit_bayes_aft = brm(
    ...
    backend = "cmdstanr",
    threads = threading(2),
    save_pars = save_pars(all = TRUE), init = ifelse(prior == "cauchy", 0, rnorm(p, 0, 0.1)),
    stanvars = stanvars
...
run_grid_experiments_mc = function(
  ...
  M = 100,
  pop_n = 100000, # Lifetable population n
  bsep = 4,
  X_rho = 0.4,
  B_ro = 0.4,
  target_snr = 2,
  ...
  for (i in 1:nrow(param_grid)) { # 5 due to stuff
    # Params to compare
    params = param_grid[i,]
    ...
    for (m in 1:M) {
      ...
      results$brms = tryCatch(
        brms_reg(df_sim = gres, p = params$p, g = params$g, method = method, prior = params$beta_prior),
        error = function(e) NULL
      )
      B_stop = Sys.time()
...
run_grid_experiments_mc(experiment_name = "ResultRun")
...
<!-- ```{r} -->
<!-- mc1 = function() { -->
<!--   capture.output(run_grid_experiments_mc(n_grid = c(100), p_grid = c(20,100), -->
<!--                                   g_grid = c(5, 10), non_zero_gri1d = c(0.5, 1), M = 200, -->
<!--                                   target_snr = 2, pop_n = 100000, bsep = 4, -->
<!--                                   X_rho = 0, method = "weibull", seed_base = 42)) -->
<!-- } -->
<!-- mc1() -->
<!-- ``` -->
...
## Method: Weibull; p = 20; $n \in \{125, 250, 500, 2000\}$; $r_{nz} \in \{ 1\}$ 
...
debug_methods(n = 400, p = 400, g = 10, method = "weibull", seed = 42, t_SNR = 2, perf_A = T, nzg = 0.3, prior = "cauchy") %>%
  system.time()
```
...
debug_methods(n = 500, p = 200, g = 10, method = "weibull", seed = 42, t_SNR = 2, perf_A = T, nzg = 1) %>%
  system.time()
```
...
debug_methods(n = 500, p = 20, g = 5, method = "weibull", seed = 42, t_SNR = 2, perf_A = T, nzg = 1) %>%
  system.time()
```
...
debug_methods(n = 500, p = 20, g = 5, method = "horseshoe", seed = 42, t_SNR = 2, perf_A = T, nzg = 1) %>%
  system.time()
```
...
debug_methods(n = 100, p = 10, g = 2, method = "weibull", seed = 42, t_SNR = 1, perf_A = T, nzg = 1) %>%
  system.time()
