...
```{r}
generate_data = function(pop_n, obs_n, p, g, ltname, betafrac, seed, force_recalc, X_plots, a = exp(-9), b = 0.085, X_rho, β_rho) {
  
  print("Generating X and βs...")
  X = generate_X(n = pop_n, p = p, g = g, constant_in_group =F, rho = 0.9, seed = seed, rho_between = 0.2, non_zero_groups = 0.5, beta_scale = 0.5, beta_denom = betafrac, X_plots = X_plots)
  print("Done!")
  
  
  print("Generating population lifetable...")
  lt = generate_population_lifetable(N_pop = pop_n, M = p, betas = -X$betas,
  ...
  df_sim = create_dataset(method = "grouped", M = p, n_obs = obs_n, G = g,
                          gsize = (p/g), lt = lt, betas = X$betas,
                          seednr = seed+1, X_plots = F, a=a, b=b,
                          betafrac = betafrac)
  
  return(list(df = df_sim, true_betas = X$betas))
}
```


```{r}
df_sim = generate_data(pop_n = 100 %>% as.integer(),
                       obs_n = 500,
                       p = 20,  #  number of βs
                       g = 4,  #  β number of groups
                       ltname = "15may",  #  File prefix
                       betafrac = 20,  # Beta shrinking factor
                       seed = 124,
                       force_recalc = T,  #  Override saved lifetable
                       X_plots = T,
                       a = exp(-9),  # Default: exp(-9)
                       b = 0.085)  # Default = 0.085
lt = read_rds("pop_lifetable_15may.rds")
