...
```{r}
generate_data = function(pop_n, obs_n, p, g, ltname, seed, force_recalc, X_plots, a = exp(-9), b = 0.085, X_rho, β_rho, non_zero_betas, X_scale, active_hazard, betasep, bscale) {
  
  print("Generating βs...")
  betas = generate_betas(p = p, g=g, rho = β_rho, rho_between = 0,
                         seed = seed, mu_u = betasep, mu_l = -betasep, beta_scale = bscale,
                         plot = T, non_zero_groups = non_zero_betas, active_hazard = active_hazard)
...
psbc_reg = function(df_sim, p, g) {
  ...
  mcmcParams = list(
    run = list(
      numReps = 5000,
      thin = 1, # Update params every n steps
      burninPerc = 0.5 # Discard the first ... steps
    ),
    tuning = list(
      mu.prop.var = 0.5,
      sigSq.prop.var = 0.01,
...
run_experiment = function(pop_n = 20000, obs_n = 1000, p = 20, g = 4, ltname, seed, nzb, bsep, X_rho) {
...
```{r}
# p = 20
rmses = run_experiment(p = 20, g = 4, ltname = "p20_may20", seed = 42, nzb = 0.75, bsep = 2, X_rho = 0)
print(rmses)

rmses = run_experiment(p = 60, g = 4, ltname = "p60_may20", seed = 43, nzb = 0.75, bsep = 2, X_rho = 0)
print(rmses)
# 
# rmses = run_experiment(p = 100, g = 4, ltname = "p100_may20", seed = 44, nzb = 0.5, bsep = 2, X_rho = 0)
# print(rmses)
# 
# rmses = run_experiment(p = 200, g = 4, ltname = "p200_may20", seed = 45, nzb = 0.5, bsep = 2, X_rho = 0)
# print(rmses)
```
...
df_sim = generate_data(pop_n = 10000, obs_n = 500, p = p, g = g,
                       ...
                       force_recalc = T, X_plots = T, a = exp(-9), b = 0.085,
                       X_rho = 0, β_rho = 0.9, non_zero_betas = nzb, X_scale = 0.2,
                       #active_hazard = 0.025, betasep = 2)
                       active_hazard = 0,
                       #active_hazard = (0*sqrt(p))/nzb/bsep,
                       betasep = bsep, bscale = 1)
